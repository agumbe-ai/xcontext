---
name: xcontext
description: Use XContext to run, redact, compress, preserve, search, and retrieve large command output, logs, test results, stack traces, diffs, or JSON without flooding Claude's context. Use proactively when output may be noisy, repetitive, larger than a few screens, or contain secrets.
---
...
1. For a command likely to produce large output, call `mcp__xcontext__xcontext_execute` with an argv array. Never combine a command through a shell string.
